...
async function add_superthanks(
  ...
  if (superThanks) {
    ...
    if (youtubeHandle != superThanks.youtubeHandle) {
      console.log(
        " Warning: youtube handle in JSON of ",
        youtubeHandle,
        " does not equal the one in the database: ",
        superThanks.youtubeHandle,
      );
    }
    if (amount != superThanks.amount) {
      console.log(
        " Warning: amount in JSON of ",
        amount,
        " does not equal the one in the database: ",
        superThanks.amount,
      );
...
async function import_superthanks_from_json(comments, commit) {
  ...
  for (let comment of comments) {
    ...
    const amount =
      comment.paidCommentChipRenderer.pdgCommentChipRenderer.chipText
        .simpleText;
...
async function sync_or_create_persons(commit: bool) {
  ...
  console.log(
    "Linking/creating persons for ",
    thanksNeedingUpdates.length,
    " records",
  );
  if (!commit) {
    return;
...
async function main() {
  if (process.argv.length < 3 || process.argv.length > 4) {
    throw new Error(
      "provide path to youtube json data, as 'npx tsx scripts/update_superthanks.ts get_comments.json'",
    );
  }
  ...
  if (process.argv.length == 4) {
    if (process.argv[3] === "--commit") {
      ...
      throw new Error(
        "unknown argument '" + process.argv[3] + "', expected '--commit'",
      );
    }
  }

  await import_superthanks_from_json(
    data.contents.itemSectionRenderer.contents,
    commit,
  );
