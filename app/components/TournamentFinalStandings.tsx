...
function TournamentFinalStandings({ tournament }) {
  ...
  for (const rnd of tournament.results) {
    ...
    if (
      summary[crgaid].best == "DNF" ||
      (rnd.resultCode == 0 &&
        rnd.elapsedMilliseconds < summary[crgaid].best_time)
    ) {
  ...
  const colDefs = [
    ...
    },
    {
      ...
      flex: 1,
      cellRenderer: (p) => {
        return <>{summary[p.data.person.crgaid].best}</>;
      },
      sortable: false,
    },
    {
      ...
      flex: 1,
      cellRenderer: (p) => {
        return <>{summary[p.data.person.crgaid].points}</>;
      },
      sortable: false,
    },
  ];
